      ...
      <div class="bg-white dark:bg-brand-card border border-slate-200 dark:border-slate-700 rounded-3xl shadow-md p-8 w-full max-w-md relative overflow-hidden transition-colors duration-300">
        ...
        <div class="text-center mb-8 relative z-10">
          <div class="w-16 h-16 bg-slate-100 dark:bg-brand-dark rounded-2xl flex items-center justify-center mx-auto mb-4 border border-slate-200 dark:border-white/10 shadow-lg transition-colors duration-300">
            <i class="fa-solid fa-lock-open text-3xl text-brand-accent"></i>
          </div>
          <h1 class="text-3xl font-bold text-slate-900 dark:text-white tracking-tight mb-2 transition-colors duration-300">{{ i18n.t('reset.title') }}</h1>
          <p class="text-slate-600 dark:text-slate-300 text-base">{{ i18n.t('reset.subtitle') }}</p>
        </div>

        <form *ngIf="!success && sessionChecked" (ngSubmit)="onSubmit()" class="space-y-5 relative z-10">
          <div>
            <label class="block text-base font-medium text-slate-700 dark:text-slate-300 mb-1.5 transition-colors duration-300">{{ i18n.t('reset.newPassword') }}</label>
            <div class="relative">
          ...
          <div>
            <label class="block text-base font-medium text-slate-700 dark:text-slate-300 mb-1.5 transition-colors duration-300">{{ i18n.t('reset.confirmPassword') }}</label>
            <div class="relative">
              <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <i class="fa-solid fa-lock text-slate-400 dark:text-slate-500"></i>
              </div>
        ...
        <div *ngIf="!sessionChecked" class="text-center py-8 relative z-10">
          ...
          <p class="text-slate-600 dark:text-slate-300">Validating access token...</p>
        </div>
        ...
        <div *ngIf="success" class="space-y-6 relative z-10 text-center">
          <div class="bg-emerald-50 dark:bg-emerald-500/10 p-4 rounded-xl border border-emerald-200 dark:border-emerald-500/20 text-emerald-600 dark:text-emerald-400 flex flex-col items-center">
            ...
            <p class="font-medium text-base">{{ i18n.t('reset.success') }}</p>
          </div>
        </div>

        <div *ngIf="sessionChecked && error && !supabase.currentUser()" class="mt-6 text-center text-base relative z-10 transition-colors duration-300">
